...
Nuxt provides the `<NuxtClientFallback>` component to render its slot in client-side if it fails to render in ssr.
You can specify a `fallbackTag` to `<NuxtClientFallback>` to make it render a specific tag if it fails to render in ssr.
