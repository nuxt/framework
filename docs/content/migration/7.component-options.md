...
1. It will handle 'smartly' making [direct API calls](/guide/key-concepts/server-engine#direct-api-calls) if it's running on the server, or making a client-side call to your API if it's running on the client. (It can also handle calling third-party APIs.)

2. Plus, it comes with convenience features including automatically parsing responses and stringifying data.

You can read more [about direct API calls](/guide/key-concepts/server-engine#direct-api-calls) or [fetching data](/getting-started/data-fetching).
