...
::alert{type="info"}
This directory is **optional**, meaning that [`vue-router`](https://router.vuejs.org) won't be included if you only use [app.vue](/api-reference/directory-structure/app), reducing your application's bundle size.
::
...
If you are using [app.vue](/api-reference/directory-structure/app), make sure to use the `<NuxtPage/>` component to display the current page:
...
You can define the layout used to render the route. This can be either false (to disable any layout), a string or a ref/computed, if you want to make it reactive in some way. [More about layouts](/api-reference/directory-structure/layouts).

#### `middleware`

You can define middleware to apply before loading this page. It will be merged with all the other middleware used in any matching parent/child routes. It can be a string, a function (an anonymous/inlined middleware function following [the global before guard pattern](https://router.vuejs.org/guide/advanced/navigation-guards.html#global-before-guards)), or an array of strings/functions. [More about named middleware](/api-reference/directory-structure/middleware).
...
To navigate between pages of your app, you should use the [`<NuxtLink>`](/api-reference/components/nuxt-link) component.
...
::alert{type="info"}
Learn more about [`<NuxtLink>`](/api-reference/components/nuxt-link) usage.
::
