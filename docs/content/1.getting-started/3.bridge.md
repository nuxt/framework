...
::alert{type=warning}
Nuxt Bridge provides identical features of Nuxt 3 ([docs](/docs/usage)) but there are some limitations notably `useAsyncData` and `useFetch` composables are not available. Please read rest of this page for details.
::
...
If you was previusly using `@vue/composition-api` or `@nuxtjs/composition-api`, please read [composition api migration guide](/getting-started/bridge-composition-api).
...
You can now migrate to Nuxt 3-style plugins API, which are slightly different in format than Nuxt 2.
...
Nuxt Bridge provides an new Nuxt 3 meta API that can be accessed with a new `useMeta` composable.
...
You will also need to enable feature explicitly in your `nuxt.config`:
