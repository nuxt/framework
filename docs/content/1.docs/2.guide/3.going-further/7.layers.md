...
Additionally, you can create more files to extend in the layer directory. They will be auto-scanned by Nuxt and extended for the project extending this layer.
...
You can use internal property `nuxt.options._layers` to support custom multi layer handling for your modules. Example:
...
- First items in the `_layers` array should have higher periority and override the next ones
- User's project itself is the first item in the `_layers` array
