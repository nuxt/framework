...
The Vue Virtual DOM (VDOM) has been rewritten from the ground up and allows for better rendering performance. On top of that, when working with compiled Single-File components, the Vue compiler can further optimize components at build time by separating static and dynamic markup.
