...
- The reactivity model of Vue, where a change in the data automatically triggers a change in the interface.
...
The [Composition API](https://vuejs.org/guide/extras/composition-api-faq.html) introduced in Vue 3 is not a replacement for the Options API, but enables better logic reuse throughout an application, and a more natural way to group code by concerns in complex components.
