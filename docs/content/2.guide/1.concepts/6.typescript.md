...
By default, Nuxt doesn't check types when you run `nuxi dev` or `nuxi build`, for performance reasons. However, you can [manually check your types with nuxi](/api-reference/commands/cli).
...
[Read more about how to extend this configuration](/api-reference/directory-structure/tsconfig).

::alert{icon=👉}
Nitro also [auto-generates types](/guide/concepts/server-engine#typed-api-routes) for API routes. Plus, Nuxt also generates types for globally available components and [auto-imports from your composables](/api-reference/directory-structure/composables), plus other core functionality.
::
::alert
