...
When your goal is to preserve state for parent routes use this syntax: `<NuxtPage :keepalive="{ include: 'index' }" />`. You can also set props to be passed to `<KeepAlive>` (see a full list [here](https://vuejs.org/api/built-in-components.html#keepalive)).
