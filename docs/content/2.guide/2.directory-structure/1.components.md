...
By default, any file with extension specified in [extensions key of `nuxt.config.ts`](/api/configuration/nuxt-config#extensions) treats as a component. \
If you need to restrict file extensions that should be registered as components, use the extended form of components directory declaration and its `extensions` key:
...
If you want to auto-import components based only on its name, not path, then you need to set `pathPrefix` option to `false` using extended form of configuration object:
