...
::alert{type=info}
If the server has rendered a page, route middleware will not run again on the first load of that page, but will run on future client-side navigation.
:::StabilityEdge{title="Single-run route middleware"}
:::
::
