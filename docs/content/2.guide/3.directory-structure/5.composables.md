...
**Method 4:** Using a composable within another composable

```js [composables/use-twoFoo.ts or composables/useTwoFoo.ts]
export const useTwoFoo = () => {
  const foo = useFoo()
  return useState('twoFoo', () => foo)
}
```
