...
You can use auto-imports [composables](https://v3.nuxtjs.org/guide/directory-structure/composables#example) within Nuxt plugins:
...
- If a composable depends on another plugin that it registered later, it won't work.
  - Reason: Plugins are called in sequence and in order of `plugins: []` in nuxt.config (or file names)
- If a composable depends on Vue.js lifecycle, it won't work
  - Normally, Vue.js composables are bound to current component instance while plugins are only bound to `nuxtApp`
