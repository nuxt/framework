...
```ts [pages/defineComponent-index.tsx]
...
- `defineComponent-index.tsx` and `Functional-index.tsx`: If you use a `window` object, you need to make a `process.client` judgment
- `mix-index.tsx`: `window` can be used in `start` functional components, and does not need to do `process.client` judgment
