...
Note that pages **must have a single root element** to allow route transitions between pages, and that HTML comments are considered elements as well.
This means that when the route is server rendered, or statically generated, you will be able to see its contents correctly, but when you navigate towards that route during client side navigation the transition between routes will fail and you'll see that the route will not be rendered.
See the code examples below for pragmatic examples of what constitutes a correctly defined page and what does not.

::code-group

```vue [pages/working-route.vue]
<template>
 <div>
...
```vue [pages/bad-route-example-1.vue]
<template>
...
```vue [pages/bad-route-example-2.vue]
<template>
 <div> This page </div>
