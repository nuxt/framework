...
Each file should export a default function defined with `eventHandler()`.
...
export default eventHandler(() => 'Test get handler')
```
...
export default eventHandler(() => `Default foo handler`)
```

```ts [/server/api/[...].ts]
export default eventHandler(() => `Default api handler`)
```
