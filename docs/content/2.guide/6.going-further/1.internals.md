...
When rendering a page in the browser or on the server side, a shared context will be created, referred to as `nuxtApp`.
...
Global usage is only possible for the browser but not possible on the server side to avoid sharing context between users.
