...
When migrating from Nuxt 2 to Nuxt 3, you will have to update how you programmatically navigate your users. In Nuxt 2, you had access to the underlying Vue Router with `this.$router`. In Nuxt 3, you will have to use the `useRouter()` composable which gives you access to the Vue Router. Make sure you set this up in your `setup()` or `<script setup>`.
