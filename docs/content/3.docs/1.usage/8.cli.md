...
The `add` command can be used to generate new elements like components, composables, layouts, plugins and many more.

Option        | Default          | Description
-------------------------|-----------------|------------------
`TEMPLATE` | `false` | Specify a template of the file to be generated. Choose from component, composable, layout, plugin, page, middleware, api.
`NAME` | `test` | Specify a name of the file that will be created.
`rootDir` | `.` | The directory of the target application.
