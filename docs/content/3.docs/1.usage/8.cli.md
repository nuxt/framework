...
```{bash}
npx nuxi module add|remove <NAME> [--cwd]
```

The `module add|remove` command updates the modules of your Nuxt project

Option        | Default          | Description
-------------------------|-----------------|------------------
`add|remove` | - | Specify if you want to add the module or remove it.
`NAME` | - | Specify a name of the file that will be created.
`--cwd` | `.` | The directory of the target application.