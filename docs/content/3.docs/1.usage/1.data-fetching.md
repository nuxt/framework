...
But in server-side, since server is making another `fetch` request, it doesn't send original browser cookies back to the api.

If you need to access request headers during server-side-rendering, you can use [`useReqHeaders`](/docs/usage/ssr) and in order to browser cookies to the API.

**Example:**

The example below adds the request headers to an isomorphic `fetch` call to ensure that the API endpoint has access to the same `cookie` header originally sent by user.

```vue
<script setup>
const { data } = useFetch('/api/me', {
  headers: useReqHeaders(['cookie'])
})
</script>
```

::alert{type="warning"}
Be very careful before proxying headers to an external API and include headers that you actually need.
Not all headers are safe to be proxified and might introduce unwanted behavior.
Here is a list of common headers are NOT to be proxified:
