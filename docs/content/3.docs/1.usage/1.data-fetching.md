...
## Accessing user headers on server-side call

If you need to access request headers or cookies on a local call to a Nitro endpoint, you can use [`useReqHeaders`](/docs/usage/ssr) to proxy them. (Without this, you won't be able to access them on the initial server-side render.) For example:
