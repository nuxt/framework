...
Invaliate the cache of `useAsyncData` and `useLazyAsyncData` and trigger the refetch.

### Usage

```ts
const refetch = useDataRefetch(keys?: string | string[])
...
* `keys`: Provides an array of keys that used in `useAsyncData` to refetch. When it's not specified, all `useAsyncData` will be refetched.
...
const refetch = useDataRefetch('count')
</script>
```
