...
```
## Changing URL Parameters with `asyncData`
...
const { data:users, loading, refresh, error } = await useAsyncData( 'users', () => 
  $fetch( 
    config.API_BASE_URL+`/users?page=${page.value}&take=6` 
  )
);
...
```
The key to making this work is to call the `refresh()` method returned from the `useAsyncData()` composable when a query parameter has changed.

## Directly calling an API Endpoint
There are instances where you may need to directly call the API. Nuxt 3 provides a globally available $fetch method that has the same API as the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch).
...
You can also abstract your API requests into re-usable methods or composables that you can call from anywhere. Loading a paginated group of users would look something like:
...
```
Now you can load data into your application from an API and pass values that would change after page load (search params, filters, pagination, etc.).
