...
## Accessing user headers during server-side rendering

When we call `fetch` in the browser, headers like `cookie` will be directly sent to the api.
But during server-side rendering, since server is making `fetch` requests, it doesn't send user cookies to the api.
For cases like authentication, we usually need to access user headers

If you need to access request headers during server-side rendering to make an api request,
you can use [`useReqHeaders`](/docs/usage/ssr) to proxy them.
(Without this, you won't be able to access them on the initial server-side render.) For example:

```vue
<script setup>
const { data } = useFetch('/api/me', {
  headers: useReqHeaders({ include: 'cookie' })
})
</script>
```

::alert{type="warning"}
Be very careful before proxying headers to an external API. It is only safe to use without `include`/`exclude` if you are calling your own local Nitro endpoint.
::
