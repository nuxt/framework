...
But during server-side-rendering, since `fetch` request is originating from server, it doesn't include user's browser cookies.

We can use [`useRequestHeaders`](/docs/usage/ssr) in order access and bypass cookies to the API from server-side.
