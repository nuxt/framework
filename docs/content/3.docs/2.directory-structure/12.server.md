...
The server directory is used to create any backend logic for your Nuxt application. It supports HMR and powerful features. Initialy this server directory needs to be created by yourself first.
Note: Ment  is the directory on the same level as the nuxt directory - not the already existing directory in the path .nuxt/dist/server/
