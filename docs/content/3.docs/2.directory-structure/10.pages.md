...
The `<NuxtLink>` component should be used for all internal links. That means for all links to the pages within your site you should use `<NuxtLink>`. It can also be used for external links, meaning that there's no need to replace any external link with `<a>`.
...
<template>
  <div>
    ...
    <NuxtLink to="https://nuxtjs.org">External Link to another page</NuxtLink>
  </div>
</template>
```
