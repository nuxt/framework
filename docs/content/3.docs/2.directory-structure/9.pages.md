...
If you want to access the route in using Composition API, there is a global `useRoute` function that will allow you to access the route just like `this.$route` in the Options API.

```js
<script setup>
// This import statement is optional since it's automatically imported by Nuxt
// but documented here so you're aware where it is coming from.
import { useRoute } from 'vue-router'

const route = useRoute()

if (route.params.group === 'admins' and !route.params.id) {
