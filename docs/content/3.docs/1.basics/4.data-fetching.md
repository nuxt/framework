...
As seen in [Concepts > Data fetching](/concepts/data-fetching), the data returned by `asyncData` will be stored inside the page payload. This mean that every key returned that is not used in your component will be added to the payload.
...
```vue
<script setup>
const { data: mountain } = await asyncData('everest', () => 
