...
A dependency with a CommonJS format, can use `module.exports` or `exports.` to provide a default export:
...
Typescript, [Node.js in native ESM mode](https://nodejs.org/api/esm.html#interoperability-with-commonjs) and bundlers such as Webpack, provide a compatibility mechanism so that we can default import such library.
...
However, because of syntax detection and complexities for syntax detection, there is always a chance that interop default fails and we end up with something like this:
