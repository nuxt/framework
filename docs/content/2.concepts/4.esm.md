...
Nuxt 3 (and Bridge) use Native ES Modules (ESM) by default and internally.

This guide helps undrestanding what are ES Modules and how to make Nuxt projects and modules compatible with it.

## Background

### CommonJS Modules

CommonJS (or in short CJS), is a format introduced by Node.js that allows sharing functionality between isolated JavaScript files. ([read more](https://nodejs.org/api/modules.html))
...
Bundlers like Webpack and Rollup, have limited support for this syntax and allow making CommonJS modules usable in Browsers.
...
However, in recent Node.js LTS releases, it is now possible to [use ESM syntax within Node](https://nodejs.org/api/esm.html). That means that Node.js itself can process JavaScript using ESM syntax, although it doesn't do it by default. The two most common ways to enable ESM syntax are:
...
When you `import` a module rather than `require` it, Node.js resolves it differently. For example, when you import `sample-library`, Node.js will look not for the `main` but for the `module` entry in that library's `package.json`.
...
1. **You can output your library using CJS syntax.**
   ...
   As long as your library only outputs CJS code, there will be no problem importing it in a native Node.js context. (This needs to be true for the package you have in place in the `module` field of your `package.json`.)
