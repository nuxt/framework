...
export default defineNuxtCommand({
  ...
  async invoke (args) {
    ...
    const updatedNuxtPackageName = (() => {
      ...
      // Switch channel only if specific flags explicitely set
      ...
      if (isNuxt3 && !installEdgeChannel) {
        // TODO: remove RC prefix after true stable release
        return 'nuxt@rc'
      }
      // Nuxt 3 Edge
      if (isNuxt3 && installEdgeChannel) {
      ...
      // TODO: remove RC prefix after stable release
      return 'nuxt@rc'
    })()
    ...
    if (upgradedVersion === currentVersion) {
      ...
      if (commitA && commitB) {
        if (isNuxt3) {
          ...
          consola.info('Changelog:', `https://github.com/nuxt/nuxt.js/compare/${commitA}...${commitB}`)
        }
      }
    }
  }
