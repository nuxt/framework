...
entry.getRenderToString = async () => {
  const { createRenderer } = await import('vue-server-renderer/build.<%= isDev ? "dev" : "prod" %>.js').then(r => r.default || r)
  const __VUE_SSR_CONTEXT__ = globalThis.__VUE_SSR_CONTEXT__ = {}
  const _renderer = createRenderer({})

  return function renderToString (component, context) {
